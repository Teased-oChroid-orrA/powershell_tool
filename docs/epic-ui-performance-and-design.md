...
| `<details>`/`<summary>` click-to-toggle (used by every `SettingsPanel` expander section and, later, every Bushing Workbench section) | **Never fires - no code path exists at all, not even a wrong one.** `handle_click`'s match arms cover `checkbox`/`radio`/`label`/`a`/`submit`/file `input` only; a `<summary>` click falls through to `_ => {}` and recurses to the parent, eventually just clearing focus. Zero hits for `"open"` (the attribute) anywhere in `blitz-dom`'s source at all - nothing in the crate ever sets or clears a `details` element's `open` attribute in response to any event. Symptom: a `<details open: false>` (or no `open` attr, which defaults closed) never opens no matter how many times its summary is clicked; a `<details open: true>` never closes. Found investigating an unrelated scroll-length complaint - collapsing Bushing Workbench sections by default (to shorten scroll distance) would have made fields permanently inaccessible if this had shipped without the fix below landing first. **Fixed** by `Expander` (`app/src/components.rs`): keeps real `<details>`/`<summary>` markup (existing CSS keeps working) but drives `open` from an explicit signal, toggled by a manual `onclick` on `summary` - the same fix shape as `Dropdown`/`onchange`, not a new pattern. | `blitz-dom-0.2.4/src/events/mouse.rs:129-249` (`handle_click`'s full match, `_ => {}` fallthrough, no `details`/`summary` arm); zero hits for `"open"` across all of `blitz-dom-0.2.4/src`. |
