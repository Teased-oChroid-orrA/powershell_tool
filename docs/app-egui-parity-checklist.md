...
| Brain map roadmap (8 approved items, all real - screenshot-verified against a live 36-file/7468-hit result set unless noted): **click-to-open** - a file-node left-click now calls `open::that` directly, not just a selection sync. **Right-click context menu** - Open/Reveal containing folder/Copy path (via `arboard`, also a previously-declared-but-unused dependency until now), closes on outside click. **Hover preview** - tooltip now includes the file's first real matched line (`LineHit::match_line`, truncated), not just a hit count. **Type-to-highlight** - a text field dims every non-matching node to 25% opacity, screenshot-confirmed. **Fit-to-view** - a real bounding-box-based recenter/rezoom, not a bare reset-to-default. **Folder clustering** - an optional third node tier (one per immediate parent folder, `\u{1F4C1}` icon, its own color), toggled without losing the file/filter tier, screenshot-confirmed real folder paths. **Color-by toggle** - Extension (existing) / Density (hit-count heat gradient) / Recency (`FileSearchResult::modified` age gradient), screenshot-confirmed Extension and Density. **Export as image** - renders topology (nodes/edges/colors, no text labels - `tiny-skia` has no font shaping, disclosed rather than silently omitted) to a PNG via `rfd`'s save dialog. **Remember pinned layouts** - manually-dragged node positions persist by stable id (`persistence.rs`'s new `graph_pinned_layout` field) across both a re-search and an app restart, restored on every graph rebuild | DONE | this pass |
...
| Design System Epic Phase 6 (hardening/verification-only): confirmed zero new crate dependencies were added across the whole epic (`git diff` of `Cargo.toml`/`Cargo.lock` from before Phase 1 to now is empty of new packages - every font/icon/component was built on dependencies already present); confirmed zero network-related code (`reqwest`/`ureq`/`http::`/CDN references) anywhere in the new `design/` module; `cargo build --release -p app-egui` succeeds clean (2m52s, unrelated to this epic); release binary 36MB, starts in under 2s, ~83MB RSS at startup - all consistent with this app's existing footprint (OCR's bundled `.rten` models alone are ~12MB), no bloat introduced. Offline/no-admin/no-telemetry requirement re-confirmed still holds | DONE | this pass |
