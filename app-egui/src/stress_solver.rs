...
pub struct StressSolverTool {
    ...
    /// formulation API") - Strong (pointwise residual) vs. Weak (energy functional) per term,
    /// see `pinn_core::messages::TrainingUpdate::formulation_kind_report`'s doc comment. Same
...
impl StressSolverTool {
    ...
    fn solution_summary_card(&self, ui: &mut egui::Ui, tokens: &Tokens) {
        ...
        card(ui, tokens, |ui| {
            ...
            // General-PINN architecture recommendations §39 (Priority 9, "weak/variational
            // formulation API") - Strong (pointwise residual) vs. Weak (energy functional,
            // the Deep Energy Method's Π=U-W_ext) per term. Static per problem, same
            // "always shown once known, never gated" treatment as the cards above.
            if !self.formulation_kind_report.is_empty() {
                ui.add_space(10.0);
                ui.separator();
                ui.add_space(6.0);
                ui.strong("Formulation kind by term");
                ui.add_space(4.0);
